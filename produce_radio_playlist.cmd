...
:: Set environment variable for Python scripts to read
set "ELEVENLABS_API_KEY=%~1"

:: Create working directory if needed
if not exist "%~dp0past" mkdir "%~dp0past"

:: Change to project directory so Claude reads CLAUDE.md
cd /d "%~dp0"

:: Invoke Claude Code with full autonomous permissions
claude --dangerously-skip-permissions -p "Execute the DJ Radio Station pipeline. Read dj-radio-station.md for the full specification. Audio source folder: \"%~2\". Playlist output folder: \"%~3\". The ElevenLabs API key is available as environment variable ELEVENLABS_API_KEY. All working files go in the past/ subdirectory. If past/dj_intros.json already exists, preserve existing intros and only generate intros for new or missing tracks. Similarly, skip TTS rendering for intro MP3s that already exist in past/tts/."
